...
def build_scene_mode_disclosure(
    ...
    if requested == "auto":
        ...
        normalized_source = "fixed_fallback"
        resolution_owner = "wsa_fixed_fallback_policy"
        confidence = "low"
        fallback_mode: str | None = resolved
    else:
        resolved = requested
        source = "explicit_user_or_cli"
        normalized_source = "explicit_user_or_cli"
        resolution_owner = "user_or_cli"
        confidence = "high"
        fallback_mode = None
    ...
    return {
        ...
        "mode_resolution_source_normalized": normalized_source,
        "effective_mode_source": normalized_source,
        "mode_resolution_owner": resolution_owner,
        "hermes_or_profile_selected": False,
        "fallback_mode": fallback_mode,
        "fallback_policy": (
            "wsa_auto_fixed_to_fact_audit_synthesis"
            if requested == "auto"
            else "not_applicable"
        ),
        "legacy_mode_resolution_source_retained": True,
...
def build_actor_contribution_summary(
    ...
    execution = _contribution_execution_summary(
        execution_mode,
        final_synthesizer,
        submitted_list,
    )
...
def _contribution_execution_summary(
    requested_mode: str | None,
    final_synthesizer: str,
    submitted_callbacks: List[Dict[str, Any]],
) -> Dict[str, Any]:
    if requested_mode == "external_confirmed":
        mode = "external_confirmed" if submitted_callbacks else "external_waiting"
    elif requested_mode in {"deterministic_mock", "external_waiting"}:
        mode = requested_mode
    elif submitted_callbacks:
        mode = "external_confirmed"
    elif final_synthesizer.startswith("wsa_local_simulated"):
        mode = "deterministic_mock"
    else:
        mode = "external_waiting"

    output_origin = {
        "deterministic_mock": "wsa_local_deterministic_simulation",
        "external_waiting": "external_runtime_pending_callback",
        "external_confirmed": "external_runtime_callback",
    }[mode]
    statement = {
        "deterministic_mock": "Outputs were generated by WSA's local deterministic simulation.",
        "external_waiting": "No external runtime callback has been reported yet.",
        "external_confirmed": "At least one external runtime callback was reported.",
    }[mode]
    return {
        "execution_mode": mode,
        "output_origin": output_origin,
        "external_callback_confirmed": mode == "external_confirmed",
        "execution_summary": statement,
    }
